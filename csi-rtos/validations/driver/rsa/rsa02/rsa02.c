...
void rsa_if_test()
{
    ...
    ASSERT_TRUE(ret == (CSI_DRV_ERRNO_RSA_BASE | DRV_ERROR_PARAMETER));
    ...
    ASSERT_TRUE(ret == (CSI_DRV_ERRNO_RSA_BASE | RSA_ERROR_DATA_BITS));

    ret = csi_rsa_config(rsa, RSA_DATA_BITS_1024, 2, NULL);
    ASSERT_TRUE(ret == (CSI_DRV_ERRNO_RSA_BASE | RSA_ERROR_ENDIAN));
...
void rsa_func_test()
{
    ...
    rsa_test_t check_list[] = {
        ...
        /**********  PKCS1 - SHA1 *********/
        {RSA_HASH_TYPE_SHA1, RSA_PADDING_MODE_PKCS1}, //CM6501 not supported
        /**********  NOPAD - SHA1 *********/
    ...
    for (i = 0; i < sizeof(check_list) / sizeof(rsa_test_t); i++) {
        ...
        if ((i == 1) || (i == 3)) {
            ...
            ASSERT_TRUE(ret == 0);
            ASSERT_TRUE(memcmp(decrypt, pass, strlen(pass)) == 0);
        }
        ...
        ret = csi_rsa_sign(rsa, modulus, private_exponent, pass, strlen(pass), sign, &out_size, padding);
        ...
        /************  Verify Test  ********************************/
        ret = csi_rsa_verify(rsa, modulus, public_exponent, pass, strlen(pass), sign, out_size, &verify, padding);

        if ((i == 0) || (i == 2)) {
            ASSERT_TRUE(verify == 1);
            printf("---%d\n", i);
        }
...
int test_rsa(void)
{
    ...
    dtest_case_info_t test_case_info_array[] = {
        {"rsa_if_test", rsa_if_test, 1},
        {"rsa_func_test", rsa_func_test, 1},
        {NULL, NULL, 0}
    };
