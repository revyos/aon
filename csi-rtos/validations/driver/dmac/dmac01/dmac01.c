...
static void test_csi_dmac_config(void)
{
    ...
    for (i = 0; i < sizeof(dmac_config_cases) / (sizeof(dmac_test_t)); i++) {
        ...
        ret = csi_dma_config_channel(dmac_config_cases[i].ch, dmac_config_cases[i].config, (void *)dma_event_cb_fun, NULL);
...
static void dma_test_mem2mem(void)
{
    ...
    ret = csi_dma_config_channel(ch, &config, (void *)dma_event_cb_fun, NULL);
    ASSERT_TRUE(ret == 0);
