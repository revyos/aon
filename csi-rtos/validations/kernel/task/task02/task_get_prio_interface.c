/*******************************************************
 *  CSI-ROTS Test Case --  csi_kernel_task_get_prio interface test
 *******************************************************/
...
void task_get_prio_interface_test()
{
    ...
    for(i=0;i<sizeof(check_list)/sizeof(handle_status_check_t);i++)
    {
        ...
        ret = csi_kernel_task_get_prio(check_list[i].handle);
        if (check_list[i].equal) {
			MYASSERT(check_list[i].status == ret);
			if(check_list[i].status == ret)
			{
				test_case_success++;
				PRINT_RESULT("task_get_prio_interface", PASS);
			}
			else
			{
				test_case_fail++;
				PRINT_RESULT("task_get_prio_interface", FAIL);
			}
        } else {
			MYASSERT(check_list[i].status != ret);
			if(check_list[i].status != ret)
