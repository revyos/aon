...
#define DDR_DEBUG LOG_D


 /********************************/

DDR_SYSREG_REG_SW_REG_S ddr_sysreg;
 //TODO: Some porting, Maybe remove later
extern void dwc_umctl_init_skip_traing(enum DDR_TYPE type, int rank_num, int speed, enum DDR_BITWIDTH bits);
...
#else
    DDR_DEBUG("unsupport lpddr4 type!!!\n");
    return NULL;
...
enum DDR_RANK_NUM get_ddr_rank_number() {
    int rank_num = dwc_get_ddr_rank_num();
    switch(rank_num) {
       case 1: return DDR_SINGLE_RANK;
       case 2: return DDR_DUAL_RANK;
       default: {
            printf("unsupported ddr rank type, use default single!!!\n");
            return DDR_SINGLE_RANK;
       }
    }
}
...
int lp4_set_ron(int ron) {
    ...
    if (type == DDR_TYPE_LPDDR4X) {
        ...
        case 120:
            ...
            break;
        default:
            DDR_DEBUG("unsupported ron config<%d>\n", (int)ron);
            return -1;
        }
    }
        ...
        DDR_DEBUG("unsupported ddr type<%d>\n", (int)type);
        return -1;
    }
...
void enable_axi_port_norst(int port) {
...
void pll_config(int speed) {
    if (speed == 4266) {
        ...
        //3200
        ddr_sysreg_wr(DDR_CFG0 + 0xc, 0x4b000000);
        ddr_sysreg_wr(DDR_CFG0 + 0x8, 0x01408501);
        udelay(2);
        ddr_sysreg_wr(DDR_CFG0 + 0xc, 0x0b000000);
    }
    else if (speed == 2133) {
        ...
        DDR_DEBUG("Reserved Pll setting\n");
#endif
        ddr_sysreg_wr(DDR_CFG0 + 0xc, 0x4b000000);
...
void ctrl_init(int rank_num, int speed, uint32_t dfilpcfg0_value) {
    ...
    if (rank_num == 2) {
#ifdef CONFIG_DDR_MSG
        DDR_DEBUG("Ctrl in dual rank mode \n");
#endif
        wr(MSTR, 0x03080020);
    }
    else {
...
#ifdef CONFIG_DDR_MSG
        DDR_DEBUG("Enter LP4 DBI OFF mode\n");
#endif
#else
...
#ifdef CONFIG_DDR_MSG
    DDR_DEBUG("RankCTL   is %0x \n", rd(RANKCTL));
    DDR_DEBUG("DRAMTMG2   is %0x \n", rd(DRAMTMG2));
    DDR_DEBUG("DFITMG0   is %0x \n", rd(DFITMG0));
    DDR_DEBUG("DFITMG1   is %0x \n", rd(DFITMG1));
    DDR_DEBUG("DRAMTMG4  is %0x \n", rd(DRAMTMG4));//[19:16] tCCD
#endif
}
...
void addrmap(int rank_num, enum DDR_BITWIDTH bits) {
    if (bits == DDR_BITWIDTH_16) {
...
#ifdef CONFIG_DDR_MSG
        DDR_DEBUG("DDR 64bit mode, 256B interleaving\n");
#endif
        wr(ADDRMAP0, 0x0004001f);  // +2
        if (rank_num == 2) {
        ...
        DDR_DEBUG("Wrong address map setting!!!\n");
    }
}

void quasi_reg_write(unsigned int reg, int wdata) {
    DWC_DDR_UMCTL2_C_STRUCT_REG_S umctl2_reg;
...
void lpddr4_selfrefresh_exit(int mode) {
    int rdata = 0;
    wr(PWRCTL, rd(PWRCTL) & (~(1 << 5))); // put sdram  into idle state
    wr(PWRCTL_DCH1, rd(PWRCTL) & (~(1 << 5))); // put sdram  into idle state
...
#ifdef CONFIG_DDR_MSG
    DDR_DEBUG("[sdram_selfrefresh_exit] STAT is :%x after selfrefresh exit\n", rdata);
#endif
}
...
void dfi_init(int dfi_init_complete_en) {
    ...
    int rdata;
#ifdef CONFIG_DDR_MSG
    DDR_DEBUG("[dfi_init]: start dfi_init \n");
#endif
...
#ifdef CONFIG_DDR_MSG
    DDR_DEBUG("[dfi_init]: dfi_init end\n");
#endif  
}
...
#ifdef CONFIG_DDR_MSG
    DDR_DEBUG("[lpddr4_enter_selfrefresh]: CH0 STAT is :%x after enter selfrefresh state\n", umctl2_reg.dwc_ddr_umctl2_c_struct_stat.u32);
#endif
    if (mode == 0) {
...
#ifdef CONFIG_DDR_MSG
        DDR_DEBUG("[lpddr4_enter_selfrefresh]: CH1 STAT is :%x after enter selfrefresh state\n", umctl2_reg.dwc_ddr_umctl2_c_struct_stat.u32);
#endif
    }
}
...
#ifdef CONFIG_DDR_MSG
    DDR_DEBUG("[lpddr4_selfrefresh_en] wait lpddr4 enter selfrefresh by slefref_en \n");
#endif
}
...
#ifdef CONFIG_DDR_MSG
    DDR_DEBUG("de-assert areset and ctrl_crst_n,ddr_phy_crst_n by sysreg or tb \n");
#endif
    //wr(0xf0000000,0xf);
    //ddr_sysreg_wr(DDR_CFG0,0x1ff0);
    //ddr_sysreg_wr(DDR_CFG0,0x1ff0);
...
#ifdef CONFIG_DDR_MSG
    DDR_DEBUG("assert areset and ctrl_rst by sysreg \n");
#endif
    ddr_sysreg.ddr_sysreg_registers_struct_ddr_cfg0.u32 = ddr_sysreg_rd(DDR_CFG0);
...
void dfi_freq_change(int dfi_freq, int skip_dram_init) {
    ...
    int rdata;
#ifdef CONFIG_DDR_MSG
    DDR_DEBUG("[dfi_freq_change]: start dfi_freq_change, target dfi_freq is %x \n", dfi_freq);
#endif
    //wr(DBG1,3);
    umctl2_reg.dwc_ddr_umctl2_c_struct_swctl.u32 = rd(SWCTL);
    umctl2_reg.dwc_ddr_umctl2_c_struct_swctl.sw_done = 0;
  ...
  while(umctl2_reg.dwc_ddr_umctl2_c_struct_dfistat.dfi_init_complete == 0)
    ...
    //wr(DBG1,0);
#ifdef CONFIG_DDR_MSG
    DDR_DEBUG("[dfi_freq_change]: dfi_freq_change, end \n");
#endif
}
...
#ifdef CONFIG_DDR_MSG
    DDR_DEBUG("DDR SOC PLL PowerDown \n");
#endif
}
...
void dfi_freq_change_enter(unsigned int dfi_freq, unsigned int skip_dram_init)
{
    DWC_DDR_UMCTL2_C_STRUCT_REG_S umctl2_reg;
    unsigned int rdata;
...
void aclk_gate_en()
{
    unsigned int tmp;
    //VI
    tmp = rd(VISYS_CLK_TEECFG);
    tmp &= ~(0x1 << 20);
    wr(VISYS_CLK_TEECFG, tmp);
    //vo
    tmp = rd(VOSYS_ACLK_TEECFG);
    tmp &= ~(0x1 << 4);
    wr(VOSYS_ACLK_TEECFG, tmp);
    //NPU
    tmp = rd(NPU_CCLK_TEECFG);
    tmp &= ~(0x1 << 5);
    wr(NPU_CCLK_TEECFG, tmp);
    //VP
    tmp = rd(VPSYS_CLK_TEECFG);
    tmp &= ~(0x1 << 12);
    wr(VPSYS_CLK_TEECFG, tmp);
    DDR_DEBUG("vi:0x%x\n", rd(VISYS_CLK_TEECFG));
    DDR_DEBUG("vo:0x%x\n", rd(VOSYS_ACLK_TEECFG));
    DDR_DEBUG("npu:0x%x\n", rd(NPU_CCLK_TEECFG));
    DDR_DEBUG("vp:0x%x\n", rd(VPSYS_CLK_TEECFG));
}
...
void lpddr4_phy_lp2_enter(int speed)
{
    ddr_sysreg_wr(DDR_CFG1, 0xa011f);   //remove core clock after xx
    //wr(SWCTL,0);
    //wr(HWLPCTL,0x430000);
    //wr(DCH1_HWLPCTL,0x430000);
    //wr(SWCTL,1);
    ...
    //d.enter sw selref
    lpddr4_enter_selfrefresh(1, 1, 0);

    //e.enter lp2,from p0 to p0
    dfi_freq_change_enter(0x0, 0x3);
    //f.dis soc pll
    ddr_soc_pll_disable();
    //stay in lp2
...
void lpddr4_phy_lp2_exit(int speed)
{
    aclk_gate_dis();
    //while(1);
    //g.enable soc pll
    ...
    //h.exit lp2
    dfi_freq_change_exit();

    //i.exit selfref
    lpddr4_selfrefresh_exit(0);

    //j.enable axi port
    enable_axi_port_norst(0x1f);
}

void lpddr4_phy_lp2_test(int speed)
{
...
void lpddr4_phy_lp3_enter(int speed)
{
    unsigned int rdata;
    ...
    //i.ddr subsys reset EN
    wr(REG_AON_DDRSYS_RST_CFG, 0x0);
    wr(REG_AON_DDRSYS_RST_CFG, 0x0);
    wr(REG_AON_DDRSYS_RST_CFG, 0x0);

    DDR_DEBUG("*****ENTER LP3 MODE :DDR SUBSYS STAY IN RST*****\n");
    //k.DDR ISO EN
    wr(REG_AON_ISO_CFG, 0x4);
    DDR_DEBUG("*****STAY IN LP3 MODE :DDR SUBSYS STAY IN RST*****\n");
    //PWD except VDDQ.TO BE DONE
    //***add power down here**//
}

void lpddr4_phy_lp3_exit(int speed)
{
    enum DDR_TYPE type = DDR_TYPE_LPDDR4X;
    enum DDR_RANK_NUM rank_num = get_ddr_rank_number();
    ...
    //n.ISO DIS
    DDR_DEBUG("*****STAY IN LP3 MODE :DDR SUBSYS RST RELEASE*****\n");
    //o.en pll
    pll_config(speed);
    udelay(20);
    //p.phy reset
...
void lpddr4_phy_lp3_test(int speed)
{
