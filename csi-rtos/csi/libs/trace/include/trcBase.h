#ifndef __TRCBASE_H__
#define __TRCBASE_H__
...
extern uint8_t excluded_objects[(TRACE_KERNEL_OBJECT_COUNT + TRACE_NCLASSES) / 8 + 1];

/* Structure to handle the exclude flags for all event codes */
extern uint8_t excluded_event_codes[NEventCodes / 8 + 1];
 ...
 *
 * The stack of handles (object_handles) is initially all zeros. Since zero
...
typedef struct
{
  ...
  uint16_t available_index[ TRACE_NCLASSES ];

  /* The start index of this class. The last one is only for check.  */
  uint16_t start_index[ TRACE_NCLASSES + 1 ];

  /* The free object handles - a set of stacks within this array */
  object_handle_t object_handles[ TRACE_KERNEL_OBJECT_COUNT ];

} object_handle_table_t;

extern object_handle_table_t object_handle_table;
...
{
  /* = NCLASSES */
  uint32_t number_of_object_classes;

  uint32_t object_property_table_size;

  /* Allocation size rounded up to the closest multiple of 4 */
  uint32_t name_length_per_class;
  ...
  object_handle_t number_of_objects_per_class[2*((TRACE_NCLASSES+1)/2)];
#else
  object_handle_t number_of_objects_per_class[4*((TRACE_NCLASSES+3)/4)];
#endif

  uint8_t total_property_bytes_per_class[ 4*((TRACE_NCLASSES+3)/4) ];

  /* Allocation size rounded up to the closest multiple of 2 */
  uint16_t start_index_of_class[ 2*((TRACE_NCLASSES+1)/2) ];

  /* The actual handles issued, should be Initiated to all zeros */
  uint8_t objbytes[ 4*((TRACE_OBJECT_TABLE_SIZE+3)/4) ];
} object_property_table_t;
...
typedef struct
{
  ...
  uint16_t latest_entry_of_checksum[64];
} symbol_table_t;
...
{
  trace_label_t name;
  trace_label_t format;
} channel_format_pair_t;

typedef struct
{
  uint16_t bufferID;
  uint16_t version;
  uint32_t wraparound_counter;
  uint32_t nslots;
  uint32_t next_slot;
  uint8_t nchannels;
  uint8_t padding1;
  uint8_t padding2;
  uint8_t padding3;
  channel_format_pair_t channels[CHANNEL_FORMAT_PAIRS+1];
  uint8_t channel_buffer[(USER_EVENT_BUFFER_SIZE + 3) & 0xFFFFFFFC]; /* 1 byte per slot, with padding for 4 byte alignment */
  uint8_t data_buffer[USER_EVENT_BUFFER_SIZE * 4]; /* 4 bytes per slot */
...
typedef struct
{
  ...
  /* sizeof(recorder_data_t) - just for control */
  uint32_t file_size;
  ...
  object_property_table_t object_property_table;
...
#else
  uint32_t float_encoding;
#endif
  /* This is non-zero if an internal error occurred in the recorder, e.g., if
  one of the Nxxx constants was too small. The system_info string will then
  ...
  uint8_t event_data[ EVENT_BUFFER_SIZE * 4 ];

#if (USE_SEPARATE_USER_EVENT_BUFFER == 1)
  user_event_buffer_t user_event_buffer;
#endif
...
extern recorder_data_t* g_recorder_data_ptr;
...
traceLabel trace_create_symbol_table_entry(const char* name,
                     uint8_t crc6,
                     uint8_t len,
                     traceLabel channel);
...
void check_data_tobe_overwritten_for_multi_entry_events(uint8_t nEntries);

object_handle_t trace_get_object_handle(traceObjectClass objectclass);

void trace_free_object_handle(traceObjectClass objectclass,
...
#else
#define trace_get_8bit_handle(x) ((unsigned char)x)
#endif


uint16_t get_index_of_object(object_handle_t objecthandle,
             uint8_t objectclass);
 ...
 ******************************************************************************/
void trace_init_trace_data(void);

/* Internal macros */

#define TRACE_PROPERTY_NAME_GET(objectclass, objecthandle) \
(const char*)(& g_recorder_data_ptr->object_property_table.objbytes \
[get_index_of_object(objecthandle, objectclass)])

#define TRACE_PROPERTY_OBJECT_STATE(objectclass, handle) \
g_recorder_data_ptr->object_property_table.objbytes[get_index_of_object(handle, objectclass) \
+ g_recorder_data_ptr->object_property_table.name_length_per_class]

#define TRACE_PROPERTY_ACTOR_PRIORITY(objectclass, handle) \
g_recorder_data_ptr->object_property_table.objbytes[get_index_of_object(handle, objectclass) \
+ g_recorder_data_ptr->object_property_table.name_length_per_class + 1]
...
#define TRACE_SET_EVENT_CODE_FLAG_ISEXCLUDED(eventCode) TRACE_SET_FLAG_ISEXCLUDED(excluded_event_codes, eventCode)
#define TRACE_CLEAR_EVENT_CODE_FLAG_ISEXCLUDED(eventCode) TRACE_CLEAR_FLAG_ISEXCLUDED(excluded_event_codes, eventCode)
#define TRACE_GET_EVENT_CODE_FLAG_ISEXCLUDED(eventCode) TRACE_GET_FLAG_ISEXCLUDED(excluded_event_codes, eventCode)
