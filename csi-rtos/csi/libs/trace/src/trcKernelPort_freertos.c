...
object_handle_t trace_get_object_number(void* handle)
{
	return (object_handle_t) ucQueueGetQueueNumber(handle);
}
...
object_handle_t trace_get_object_number(void* handle)
{
	return (object_handle_t) uxQueueGetQueueNumber(handle);
}
...
{
	return (object_handle_t)uxTaskGetTaskNumber(handle);
}
...
void trace_init_object_property_table()
{
	...
	g_recorder_data_ptr->object_property_table.number_of_objects_per_class[5] = NTimer;
	g_recorder_data_ptr->object_property_table.number_of_objects_per_class[6] = NEventGroup;	
	...
	g_recorder_data_ptr->object_property_table.total_property_bytes_per_class[5] = PropertyTableSizeTimer;
	g_recorder_data_ptr->object_property_table.total_property_bytes_per_class[6] = PropertyTableSizeEventGroup;
	...
	g_recorder_data_ptr->object_property_table.start_index_of_class[5] = StartIndexTimer;
	g_recorder_data_ptr->object_property_table.start_index_of_class[6] = StartIndexEventGroup;
...
void trace_init_object_handle_stack()
{
	...
	object_handle_table.available_index[TRACE_CLASS_TIMER] = NQueue + NSemaphore + NMutex + NTask + NISR;
	object_handle_table.available_index[TRACE_CLASS_EVENTGROUP] = NQueue + NSemaphore + NMutex + NTask + NISR + NTimer;
  ...
  object_handle_table.start_index[TRACE_CLASS_TIMER] = NQueue + NSemaphore + NMutex + NTask + NISR;
  object_handle_table.start_index[TRACE_CLASS_EVENTGROUP] = NQueue + NSemaphore + NMutex + NTask + NISR + NTimer;
}
