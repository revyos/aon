...
{
    uint32_t time;
	/* Modify time calculation for execution efficiency */
    //time = ((((uint64_t)csi_coret_get_valueh() << 32U) | csi_coret_get_value())) / (soc_get_coretim_freq() / 1000U);
	time = (((uint64_t)csi_coret_get_valueh() << 32U) | csi_coret_get_value()) * 3 >> 13U;
    return time;
}
...
void udelay(uint32_t us)
{
    ...
    uint32_t starth = csi_coret_get_valueh();
    uint32_t curl, curh;
    uint32_t cnt = us * (soc_get_coretim_freq() / 1000000U);
    start = ((unsigned long long)starth << 32U) | startl;
