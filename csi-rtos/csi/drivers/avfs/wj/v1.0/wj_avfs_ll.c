								...
								uint32_t            pmic_reg_val)
{
	...
	uint32_t       cmd = 0;
	cmd = WJ_AVFS_VOL_CMD_GRP_PARAM_CMD_VLD | \
	      (i2c_reg << WJ_AVFS_VOL_CMD_GRP_PARAM_ADDR_POS) | \
		  pmic_reg;
	p_grp_base[cmd_idx*2+0] = cmd;
	cmd = WJ_AVFS_VOL_CMD_GRP_PARAM_CMD_VLD | \
	      WJ_AVFS_VOL_CMD_GRP_PARAM_VLTG_VLD | \
	      (i2c_reg << WJ_AVFS_VOL_CMD_GRP_PARAM_ADDR_POS) | \
		  pmic_reg_val;
	p_grp_base[cmd_idx*2+1] = cmd;
	return;
}								
								...
								uint32_t              pmic_reg_val)
{
	...
	__IOM uint32_t *p_grp_base = (grp_num == AVFS_PMIC_CMD_GRP_A) ? avfs_base->vol_base.PMIC_CMD_A : avfs_base->vol_base.PMIC_CMD_B;
	cmd = WJ_AVFS_VOL_CMD_GRP_PARAM_CMD_VLD | \
	      ((bvltg) ? WJ_AVFS_VOL_CMD_GRP_PARAM_VLTG_VLD : 0) | \
	      (pmic_reg << WJ_AVFS_VOL_CMD_GRP_PARAM_ADDR_POS) | \
		  pmic_reg_val;
...
void wj_avfs_vol_spi_write_reg(wj_avfs_ctrl_regs_t *avfs_base, uint8_t reg_addr, uint8_t reg_val)
{
	avfs_base->vol_base.PMIC_SPI_CMD0 = WJ_AVFS_VOL_SPI_SW_SPI_EN_EN | \
										...
										WJ_AVFS_VOL_SPI_CMD0_CMD_RW_MASK | \
										reg_val;
...
void wj_avfs_cmu_pll_set_switch_param(wj_avfs_ctrl_regs_t *avfs_base,uint8_t *clk_switch_item,uint32_t item_num)
{
	...
	{
		avfs_base->freq_base.PLL_SWITCH_CFG[index >> 2] &= ~(0xff << (index & 0x3));
		avfs_base->freq_base.PLL_SWITCH_CFG[index >> 2] |= (*clk_switch_item << (index & 0x3));
		index++;
		clk_switch_item++;
	}	
}
								  ...
								  uint16_t    item_num)
{
	...
	csi_error_t ret;
    /* check read item */
	if(type == PVT_TYPE_POWER && (item_start + item_num) > PS_DATA_LEN) {
		return CSI_ERROR;
	}
...
void wj_avfs_sensor_ts_config(wj_avfs_ctrl_regs_t *avfs_base,bool bhwauto,ts_sample_type_t type,bool bauto_freq_adjust)
{
	...
	cfg = ((bhwauto) ? WJ_AVFS_TSENSOR_CFG_MGR_MASK : 0 ) | \
	      (tmp) | \
