...
/* offset 0x00: WDATA REG */

/* offset 0x04: CMD   REG */
#define WJ_CHAINCTRL_CMD_REG_INDEX_POS                                 (0U)
#define WJ_CHAINCTRL_CMD_REG_INDEX_MASK                                (0xF << WJ_CHAINCTRL_CMD_REG_INDEX_POS)

#define WJ_CHAINCTRL_CMD_SENSOR_INDEX_POS                              (4U)
#define WJ_CHAINCTRL_CMD_SENSOR_INDEX_MASK                             (0x1F << WJ_CHAINCTRL_CMD_SENSOR_INDEX_POS)

#define WJ_CHAINCTRL_CMD_TYPE_POS                                      (12U)
#define WJ_CHAINCTRL_CMD_TYPE_MASK                                     (0x7 << WJ_CHAINCTRL_CMD_TYPE_POS)
#define WJ_CHAINCTRL_CMD_TYPE_SINGLE_READ                              (0U)
#define WJ_CHAINCTRL_CMD_TYPE_SINGLE_WRITE                             (1U)
#define WJ_CHAINCTRL_CMD_TYPE_SERIAL_READ                              (2U)
#define WJ_CHAINCTRL_CMD_TYPE_SERIAL_WRITE                             (3U)
#define WJ_CHAINCTRL_CMD_TYPE_DATA                                     (4U)

#define WJ_CHAINCTRL_CMD_VLD_POS                                       (16U)
#define WJ_CHAINCTRL_CMD_VLD_MASK                                      (0x1 << WJ_CHAINCTRL_CMD_VLD_POS)
#define WJ_CHAINCTRL_CMD_VLD_EN                                        WJ_CHAINCTRL_CMD_VLD_MASK

#define WJ_CHAINCTRL_CMD_READY_POS                                     (17U)
#define WJ_CHAINCTRL_CMD_READY_MASK                                    (0x1 << WJ_CHAINCTRL_CMD_READY_POS)

typedef struct {
    __IOM uint32_t  WDATA;             /* Offset: 0x000  data reg for write op */
    __IOM uint32_t  CMD;               /* Offset: 0x004  cmd param reg         */
}wj_avfs_chainctrl_regs_t;
...
#define WJ_AVFS_VOL_CFG_STEP_POS                                       (0U)
#define WJ_AVFS_VOL_CFG_STEP_MASK                                      (0xff << WJ_AVFS_VOL_CFG_STEP_POS)

#define WJ_AVFS_VOL_CFG_MARGIN_POS                                     (8U)
...
#define WJ_AVFS_VOL_CMD_GRP_PARAM_VLTG_VLD_POS                         (30U)
#define WJ_AVFS_VOL_CMD_GRP_PARAM_VLTG_VLD_MASK                        (1U << WJ_AVFS_VOL_CMD_GRP_PARAM_VLTG_VLD_POS)
#define WJ_AVFS_VOL_CMD_GRP_PARAM_VLTG_VLD                             WJ_AVFS_VOL_CMD_GRP_PARAM_VLTG_VLD_MASK
...
typedef enum
{
	PS_SAMPLE_WSUM,                                                    ///<  weighted sum
	PS_SAMPLE_MIN,                                                     ///<  MIN VALUE
}ps_sample_type_t;
...
#define WJ_AVFS_CGM_INC_MOD_REQ_POS                                   (1U)
...
#define WJ_AVFS_CGM_FIX_MOD_VLD_POS                                   (2U)
...
#define WJ_AVFS_INTR_MASK                                             (0xffff)
#define WJ_AVFS_INTR_SW_DONE_MASK                                     (1U << 0U)
...
typedef struct 
{
	...
	__IOM uint32_t PMIC_MEM_LOGIC_VLTG;                   /*offset:0x17c*/
...
typedef struct 
{
	...
	uint32_t       RESERVED3[52];                         /*offset:0x238~0x2f8*/
...
typedef struct 
{
	#define CGM_INC_PATTERN_NUM                           (16U)
	#define CGM_PLL_GEAR_CONFIG_NUM                       (9U)
...
typedef struct 
{
	__IOM uint32_t FPV_TH;
	__IOM uint32_t FPV_TAR;
}fpv_param_t;
...
typedef struct 
{
	#define FPV_TABLE_LEN                                (9U)
	#define PS_WEIGHT_LEN                                (32U)
	#define PS_DATA_LEN                                  (32U)
	#define VS_DATA_LEN                                  (8U)
	fpv_param_t fpv[FPV_TABLE_LEN];                      /* 0x800 ~ 0x844 */
	uint32_t RESERVED[14];
	__IOM uint32_t PS_LOW_WEIGHT[PS_WEIGHT_LEN>>1];      /* 0x880 ~0x8bc  */
	__IOM uint32_t PS_NOR_WEIGHT[PS_WEIGHT_LEN>>1];      /* 0x8c0 ~0x8fc  */
	__IOM uint32_t PS_HIGH_WEIGHT[PS_WEIGHT_LEN>>1];     /* 0x900 ~0x93c  */
...
typedef struct {
	...
	uint32_t       RESERVED6[64];                         /*offset:0x600~0x700*/
...
static inline void wj_avfs_misc_config_fpv(wj_avfs_ctrl_regs_t *avfs_base,uint32_t index,uint16_t pth,uint16_t v_tar)
{
	avfs_base->mem_data.fpv[index].FPV_TH = pth;
	avfs_base->mem_data.fpv[index].FPV_TAR = v_tar;
}
...
static inline void wj_avfs_chainctrl_write_data(wj_avfs_chainctrl_regs_t *chainctrl_base,uint32_t val)
{
	chainctrl_base->WDATA = val&0xff;
}

static inline void wj_avfs_chainctrl_set_sensor_param(wj_avfs_chainctrl_regs_t *chainctrl_base, uint32_t sensor_idx,uint32_t reg_idx)
{
	chainctrl_base->CMD &=~(WJ_CHAINCTRL_CMD_REG_INDEX_MASK|WJ_CHAINCTRL_CMD_SENSOR_INDEX_MASK);
	chainctrl_base->CMD |= ((sensor_idx&0x1f)<< WJ_CHAINCTRL_CMD_REG_INDEX_POS) | \
	                       ((reg_idx&0xf)<< WJ_CHAINCTRL_CMD_REG_INDEX_POS);
}

static inline void wj_avfs_chainctrl_set_access_mode(wj_avfs_chainctrl_regs_t *chainctrl_base,uint32_t cmd_type)
{
	chainctrl_base->CMD &=~WJ_CHAINCTRL_CMD_TYPE_MASK;
	chainctrl_base->CMD |= (cmd_type&0x7) << WJ_CHAINCTRL_CMD_TYPE_POS;
}

static inline void wj_avfs_chainctrl_send_cmd(wj_avfs_chainctrl_regs_t *chainctrl_base)
{
	chainctrl_base->CMD |= WJ_CHAINCTRL_CMD_VLD_EN;
}

static inline uint32_t wj_avfs_chainctrl_is_cmd_done(wj_avfs_chainctrl_regs_t *chainctrl_base)
{
	return (chainctrl_base->CMD & WJ_CHAINCTRL_CMD_READY_MASK) ? 1 : 0;
}
...
static inline void wj_avfs_vol_cfg_step_value(wj_avfs_ctrl_regs_t *avfs_base,uint32_t step_val)
{
	...
	avfs_base->vol_base.VOL_CFG = (temp & (~WJ_AVFS_VOL_CFG_STEP_MASK)) | (step_val & 0xff);
}
...
static inline void wj_avfs_vol_set_memarray_logic_vltg(wj_avfs_ctrl_regs_t *avfs_base, uint32_t vltg_low,uint32_t vltg_high)
{
	avfs_base->vol_base.PMIC_MEM_LOGIC_VLTG = (vltg_low & 0xff) | ((vltg_high & 0xff) << 8);
}
...
{
	avfs_base->vol_base.PMIC_DUAL_RAIL_ENA = 0;
}
...
static inline void wj_avfs_sensor_cfg_mgr_en(wj_avfs_ctrl_regs_t *avfs_base, bool mgr_ena)
{
	if(mgr_ena) {
...
static inline void wj_avfs_sensor_vs_cofnig(wj_avfs_ctrl_regs_t *avfs_base, bool bhwauto)
{
	avfs_base->sensor_base.VSENSOR_CFG = (bhwauto) ? 0 : 1;
}
...
static inline void wj_avfs_cgm_ctrl_inc_mode_req_disable(wj_avfs_ctrl_regs_t *avfs_base)
{
	avfs_base->freq_base.CGM_CTRL &= ~WJ_AVFS_CGM_INC_MOD_REQ_EN;
}
...
static inline void wj_avfs_cgm_ctrl_inc_mode_disable(wj_avfs_ctrl_regs_t *avfs_base)
{
	avfs_base->freq_base.CGM_CTRL &= ~WJ_AVFS_CGM_FIX_MOD_VLD_EN;
}
...
void wj_avfs_vol_config_spi_cmd(wj_avfs_ctrl_regs_t * avfs_base,
                                pmic_cmd_grp_t        grp_num,
								uint32_t              cmd_idx,
								bool                  bvltg,
								uint32_t              pmic_reg,
								uint32_t              pmic_reg_val);
void wj_avfs_vol_spi_write_reg(wj_avfs_ctrl_regs_t *avfs_base, uint8_t reg_addr, uint8_t reg_val);

uint8_t wj_avfs_vol_spi_read_reg(wj_avfs_ctrl_regs_t *avfs_base,uint8_t reg_addr);
...
void wj_avfs_sensor_ts_config(wj_avfs_ctrl_regs_t *avfs_base,bool bhwauto,ts_sample_type_t type,bool bauto_freq_adjust);
