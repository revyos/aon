...
csi_error_t csi_qspi_init(csi_qspi_t *qspi, uint32_t idx)
{
    ...
    if (target_get(DEV_DW_QSPI_TAG, idx, &qspi->dev) != CSI_OK) {
        ...
        dw_qspi_config_rx_sample_delay(qspi_base,0);
    }
...
static csi_error_t dw_qspi_send_dma(csi_qspi_t *qspi, csi_qspi_command_t *cmd, const void *data, uint32_t size)
{
    ...
    do {
		...
		/* non-standard spi mode (dual/quad) */
            ...
            /* data transfer */
			#ifdef SINGLE_BLK
			uint32_t *p_cmd_addr = NULL;
            qspi->tx_size = tx_temp_size = temp_xfer.xfer_post_len+ (DW_QSPI_DMA_CMD_ADDR_SPACE<<2);
            qspi->tx_data = temp_xfer.xfer_post-(DW_QSPI_DMA_CMD_ADDR_SPACE<<2);
			p_cmd_addr = (uint32_t*)qspi->tx_data;
			p_cmd_addr[0] = cmd->instruction.value;
			p_cmd_addr[1] = cmd->address.value;
			#else
