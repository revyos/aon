...
/**
  \brief      clear wdt interupt flag
  \param[in]  wdt    handle wdt handle to operate
  \return     void
*/
void csi_wdt_intr_clear(csi_wdt_t *wdt)
{
    dw_wdt_regs_t *wdt_base = (dw_wdt_regs_t *)HANDLE_REG_BASE(wdt);
    dw_wdt_eoi_intr_clear(wdt_base);
}

/**
  \brief       Check wdt is timeout
  \param[in]   wdt    handle wdt handle to operate
  \return      true->timeout, false->not timeout
*/
bool csi_wdt_is_timeout(csi_wdt_t *wdt)
{
    CSI_PARAM_CHK(wdt, false);

    dw_wdt_regs_t *wdt_base = (dw_wdt_regs_t *)HANDLE_REG_BASE(wdt);

    return (dw_wdt_stat_status_get(wdt_base) ? true : false);
}
